...
try:
    ...
    st.error(f"⚠️ File mancante: {e.filename}")
else:
    # Filtra solo bus OP1
    ...
    # Leggi stop_times a blocchi e raccogli tutti gli stop_id associati
    stop_ids_set = set()
    ...
    try:
        ...
        st.warning("⚠️ File stop_times.txt non trovato.")
        stop_ids_set = set()
    ...
    if stop_ids_set:
        ...
        st.warning("⚠️ Nessuna fermata trovata per questa linea.")
...
# Esempio di dataset esteso con day_of_week e week_range
...
# -----------------------------
# Visualizzazione dei due file del modello prescrittivo
# -----------------------------
st.subheader("Output del modello prescrittivo: ottimizzazione delle corse (fasce orarie multiple)")

# Percorsi fissi dei due file
file1 = r"C:/Users/C.Marino/Desktop/dataset/ottimizzazione_dashboard_20250325_112654.csv"
file2 = r"C:/Users/C.Marino/Desktop/dataset/ottimizzazione_dashboard_20250325_113839.csv"
...
# Percorso cartella GTFS
gtfs_path = r"C:/Users/C.Marino/Desktop/dataset"

try:
    # Carica GTFS se non già caricati
    ...
    #  Normalizza route_id per compatibilità con GTFS (es. "Linea 73" → "73")
    ...
    for idx, row in corse_filtrate.iterrows():
        ...
        if not trips_match.empty:
            trip_id = trips_match['trip_id'].iloc[0]  # Prima corsa disponibile
            ...
            fermate_linea['route_id'] = route_id_display  # Mostriamo "Linea 73", ecc.
